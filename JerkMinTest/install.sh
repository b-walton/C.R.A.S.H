...
echo "=========================="
echo "Installing App JerkMinTest"

# Here is where we perform installation of scripts, services, etc.
echo " - Installing ROS packages for JerkMinTest..."
...
runuser -l $LIBPANDA_USER -c /etc/libpanda.d/apps/JerkMinTest/installRosPackages.sh
...
rosrun robot_upstart install C.R.A.S.H/launch/${LAUNCH_FILE} --user root

echo "Enabling can_to_ros startup script"
sudo systemctl daemon-reload
sudo systemctl enable C.R.A.S.H
popd
echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
